...
public:
    bool isScramble(string s1, string s2) {
        ...
        return helper(s1,s2,u);
    }
    bool helper(string s1, string s2,unordered_map<string,int> &u)
    {
        if(s1==s2)
            return true;
        ...
        {
            bool c1=(helper(s1.substr(0,k+1),s2.substr(0,k+1),u) && helper(s1.substr(k+1),s2.substr(k+1),u));
            bool c2=(helper(s1.substr(0,k+1),s2.substr(s2.length()-k-1),u) && helper(s1.substr(k+1),s2.substr(0,s2.length()-k-1),u));
            if(c1 || c2)
            {
                ans=true;
                break;
